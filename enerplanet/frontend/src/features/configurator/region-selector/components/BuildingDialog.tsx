...
const ReadOnlyRow: FC<{ label: string; value: React.ReactNode; unit?: string }> = ({
  label,
  value,
  unit,
}) => (
...
export const BuildingDialog: FC<BuildingDialogProps> = ({
  ...
  const [editedHouseholdSize, setEditedHouseholdSize] = useState<number>(
    Math.max(
      1,
      Math.round(selectedBuilding?.householdSize ?? selectedBuilding?.cbsAvgHouseholdSize ?? 2)
    )
  );
  ...
  useEffect(() => {
    ...
    const preferred =
      ...
      "unknown";
    setSelectedFClass(
      fClassOptions.includes(preferred) ? preferred : (fClassOptions[0] ?? preferred)
    );

    const f = selectedBuilding?.floors3dBag ?? selectedBuilding?.floors ?? 1;
    setEditedFloors(f);
    setEditedArea(Math.round(selectedBuilding?.area ?? 0));
    setEditedHouseholdSize(
      Math.max(
        1,
        Math.round(selectedBuilding?.householdSize ?? selectedBuilding?.cbsAvgHouseholdSize ?? 2)
      )
    );
    setEditedEnergyLabel(selectedBuilding?.energyLabel?.trim().toUpperCase() || "");
    setHotWaterElectric(false);
    previousOsmIdRef.current = currentOsmId;
  }, [
    selectedBuilding?.osmId,
    selectedBuilding?.floors3dBag,
    selectedBuilding?.floors,
    selectedBuilding?.area,
    selectedBuilding?.householdSize,
    selectedBuilding?.cbsAvgHouseholdSize,
    selectedBuilding?.selectedFClass,
    selectedBuilding?.fClass,
  ...
  const handleFClassChange = useCallback(
    (value: string) => {
  ...
  const handleHouseholdSizeEdit = useCallback(
    (val: number) => {
      const clamped = Math.max(1, Math.round(val));
      ...
      <AlertDialogContent className="max-w-md !duration-300 !animate-in !fade-in-0 !zoom-in-100 !slide-in-from-top-0">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="sr-only">
            View and edit details for the selected building, including type, energy demand, and
            technologies.
          </AlertDialogDescription>
        </AlertDialogHeader>
          ...
          <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs">
              <span>
                ...
                <span className="font-medium">{selectedBuilding.osmId}</span>
              </span>
              {selectedBuilding.fClass && (
                <span>
                  <span className="text-muted-foreground">Class:</span>{" "}
                  <span className="font-medium capitalize">{selectedBuilding.fClass}</span>
                </span>
              )}
            </div>
            ...
            <div className="pt-2 border-t space-y-2">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={editedFloors}
                    onChange={(e) => {
                      setEditedFloors(Math.max(1, Math.round(Number(e.target.value))));
                      onSave?.();
                    }}
                    min={1}
                    className="w-full px-2 py-1 border border-border rounded bg-background text-sm tabular-nums"
              ...
              {(hasFiniteNumber(selectedBuilding.heightMedian) ||
                ...
                <div className="mt-1 space-y-1 rounded-md border border-border bg-muted/30 px-3 py-2">
                  <span className="text-xs font-medium text-muted-foreground">
                    {isNL ? "3D BAG Heights" : isCZ ? "CUZK Heights" : "Building Heights"}
                  </span>
                  {hasFiniteNumber(selectedBuilding.heightMedian) && (
                    <ReadOnlyRow
                      label="Height Median"
                      value={selectedBuilding.heightMedian.toFixed(1)}
                      unit="m"
                    />
                  )}
                  {hasFiniteNumber(selectedBuilding.heightMax) && (
                    <ReadOnlyRow
                      label="Height Max"
                      value={selectedBuilding.heightMax.toFixed(1)}
                      unit="m"
                    />
                  )}
                  {hasFiniteNumber(selectedBuilding.heightGround) && (
                    <ReadOnlyRow
                      label="Height Ground"
                      value={selectedBuilding.heightGround.toFixed(1)}
                      unit="m"
                    />
                  )}
                </div>
              ...
              <div className="mt-1 space-y-1">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex gap-1 flex-wrap p-0.5">
                    {ENERGY_LABELS.map((label) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setEditedEnergyLabel(editedEnergyLabel === label ? "" : label)
                        }
                  ...
                  {selectedBuilding.energyLabel &&
                    selectedBuilding.energyLabel !== editedEnergyLabel && (
                      <span className="text-[10px] text-muted-foreground">
                        (detected: {selectedBuilding.energyLabel})
                      </span>
                    )}
                </div>
                {hasFiniteNumber(selectedBuilding.energyIndex) && (
                  <div className="text-xs text-muted-foreground">
                    Energy Index:{" "}
                    <span className="font-medium">{selectedBuilding.energyIndex.toFixed(2)}</span>
                  </div>
                )}
              </div>
              ...
              {onRecalculateDemand && (
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-[10px] text-muted-foreground">
                    {editedFloors} fl × {editedArea} m² ={" "}
                    {(editedFloors * editedArea).toLocaleString()} m²
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="pt-2 border-t space-y-2 relative">
              ...
              {showTechPicker && (
                <div
                  ref={techPickerRef}
                  className="absolute right-0 top-8 z-50 w-56 overflow-hidden rounded-lg border border-border bg-background shadow-xl"
                >
                    ...
                    <div className="space-y-1 text-xs">
                      ...
                      {hasFiniteNumber(selectedBuilding.cbsPopulation) && (
                        <div className="flex justify-between">
                          ...
                          <span>{selectedBuilding.cbsPopulation.toLocaleString()}</span>
                        </div>
                      )}
                      {hasFiniteNumber(selectedBuilding.cbsHouseholds) && (
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Households</span>
                          <span>{selectedBuilding.cbsHouseholds.toLocaleString()}</span>
                        </div>
                      )}
                      {hasFiniteNumber(selectedBuilding.cbsAvgHouseholdSize) && (
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Avg household size</span>
                          <span>{selectedBuilding.cbsAvgHouseholdSize.toFixed(2)}</span>
                        </div>
                      )}
                    </div>
                  )}
                ...
                {usesEubucco && (
                  <div className="text-xs text-muted-foreground space-y-0.5">
                    ...
                    <p className="text-[10px]">
                      ...
                      <a
                        href="https://doi.org/10.1038/s41597-023-02040-2"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="underline hover:text-foreground"
                      >
