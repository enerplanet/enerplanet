...
} from "@/features/technologies/services/technologyService";
import {
  staticDescriptions,
  optionDescriptions,
  inverterTypeEfficiency,
} from "./parameterDescriptions";
            ...
            <div className="space-y-0.5">
              {filteredOptions.map((option) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`flex items-center justify-center w-7 h-7 rounded transition-colors ${isSelected ? "bg-primary/20" : "bg-muted"}`}
                    >
                      <Fan
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span
                          className={`text-sm truncate ${isSelected ? "font-semibold text-foreground" : "font-medium text-foreground"}`}
                        >
                          {option}
                        </span>
                      </div>
...
const HubHeightSelect: FC<HubHeightSelectProps> = ({ value, options, onChange, error = false }) => {
  return (
...
export const TechParameterDialog: FC<TechParameterDialogProps> = ({
  ...
  const getParameterName = useCallback(
    (paramKey: string, fallbackAlias: string): string => {
  ...
  const getDescription = useCallback(
    (paramKey: string, alias: string, customDescription?: string): string => {
      ...
      if (parameterDescriptionKeys[paramKey]) {
        return t(parameterDescriptionKeys[paramKey], {
          defaultValue: `${alias} - Configure this parameter based on your requirements.`,
        });
      }
  ...
  const handleTurbineTypeChange = useCallback(
    (turbineName: string) => {
    ...
    if (Object.keys(dynamicOptions).length > 0 && Object.keys(windTurbineData).length > 0) {
      setConstraintValues((prev) => {
        ...
        if (turbineOptions && turbineOptions.length > 0) {
          ...
          const isValidTurbineId =
            typeof currentTurbineValue === "number" &&
  ...
  const validateConstraint = (
    ...
    if (typeof numValue === "number" && !Number.isNaN(numValue)) {
      ...
      if (
        constraint.max !== null &&
        constraint.max !== "INF" &&
        numValue > Number(constraint.max)
      ) {
  ...
  const handleValueChange = (key: string, value: string | number) => {
    ...
    if ((key === "system_capacity" || key === "cont_energy_cap_max") && typeof value === "number") {
      const partnerKey = key === "system_capacity" ? "cont_energy_cap_max" : "system_capacity";
      setConstraintValues((prev) => {
        const next: Record<string, number | string> = { ...prev, [key]: value };
        if (partnerKey in prev) {
          const prevPartner = prev[partnerKey];
          const prevSelf = prev[key];
          if (
            prevPartner === prevSelf ||
            (typeof prevPartner === "number" && prevPartner < value)
          ) {
            next[partnerKey] = value;
          }
        }
        return next;
      });
      // Clear errors for both fields
      if (errors[key] || errors[partnerKey]) {
        setErrors((prev) => {
          const newErrors = { ...prev };
          delete newErrors[key];
          delete newErrors[partnerKey];
          return newErrors;
        });
      }
      return;
  ...
  const configConstraints = technology.constraints.filter(
    (c) => !c.key.startsWith("cost_") && !c.key.startsWith("cont_")
  );
        ...
        <td className="px-3 py-2.5">
          <div className="flex items-center gap-1.5">
            ...
            <Tooltip>
              ...
              <TooltipContent
                side="top"
                className="max-w-sm p-2.5 bg-popover border border-border rounded-lg shadow-lg"
              >
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-muted/50">
                <th className="px-3 py-2 text-left text-[11px] font-semibold text-foreground">
                  Parameter
                </th>
                <th className="px-3 py-2 text-right text-[11px] font-semibold text-foreground w-32">
                  Value
                </th>
                <th className="px-3 py-2 text-right text-[11px] font-semibold text-foreground w-20">
                  Unit
                </th>
                <th className="px-3 py-2 text-right text-[11px] font-semibold text-foreground w-24">
                  Range
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">{constraints.map(renderConstraintRow)}</tbody>
          </table>
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      <AlertDialogContent ref={contentRef} className="sm:max-w-2xl p-5">
        <AlertDialogDescription className="sr-only">
          Configure technology parameters for this building
        </AlertDialogDescription>
        <Button
          ref={closeButtonRef}
          disabled={isSaving}
