...
} from "@/components/workspace/utils/groupUtils";
...
import {
  getAccessLevelColor,
  getAccessLevelName,
  getAccessLevelIconColor,
  isExpert,
  isManager,
  isExpertOrManager,
} from "@/features/admin-dashboard/utils/accessLevelUtils";
...
export const UserManagement = ({ onUsersMutated }: UserManagementProps) => {
  ...
  const apiTokensDialog = useDialog<User>();
  ...
  const { selectedUsers, isSelected, handleSelectUser, handleSelectAll, clearSelection } =
  ...
  useEffect(() => {
    clearSelection();
  }, [debouncedSearch, selectedGroup, page, clearSelection]);
  ...
  const createUser = async (userData: UserFormData): Promise<boolean> => {
    try {
      ...
      if (data.success) {
        ...
        setNotification({
          open: true,
          message: t("userManagement.notifications.userCreated"),
          severity: "success",
        });
      ...
      setNotification({
        open: true,
        message: t("userManagement.notifications.failedToCreate"),
        severity: "error",
      });
      return false;
    }
  ...
  const updateUser = async (
    ...
    try {
      ...
      if (data.success) {
        ...
        setNotification({
          open: true,
          message: t("userManagement.notifications.userUpdated"),
          severity: "success",
        });
        if (onUsersMutated) onUsersMutated();
        return true;
      ...
      setNotification({
        open: true,
        message: t("userManagement.notifications.failedToUpdate"),
        severity: "error",
      });
      return false;
    }
  ...
  const deleteUser = async (userId: string | number): Promise<boolean> => {
    try {
      ...
      if (data.success) {
        ...
        setNotification({
          open: true,
          message: t("userManagement.notifications.userDeleted"),
          severity: "success",
        });
        if (onUsersMutated) onUsersMutated();
        return true;
      ...
      setNotification({
        open: true,
        message: t("userManagement.notifications.failedToDelete"),
        severity: "error",
      });
      return false;
    }
  ...
  const disableUser = async (userId: string | number): Promise<boolean> => {
    try {
      ...
      if (data.success) {
        ...
        setNotification({
          open: true,
          message: t("userManagement.notifications.userDisabled"),
          severity: "success",
        });
        if (onUsersMutated) onUsersMutated();
        return true;
      ...
      setNotification({
        open: true,
        message: t("userManagement.notifications.failedToDisable"),
        severity: "error",
      });
      return false;
    }
  ...
  const enableUser = async (userId: string | number): Promise<boolean> => {
    try {
      ...
      if (data.success) {
        ...
        setNotification({
          open: true,
          message: t("userManagement.notifications.userEnabled"),
          severity: "success",
        });
        if (onUsersMutated) onUsersMutated();
        return true;
      ...
      setNotification({
        open: true,
        message: t("userManagement.notifications.failedToEnable"),
        severity: "error",
      });
      return false;
    }
  ...
  const getAccessLevelIcon = (level: string) => (
    <Shield className={`w-3 h-3 ${getAccessLevelIconColor(level)}`} />
  );
    ...
    try {
      await confirm({
        ...
        onConfirm: async () => {
          ...
          setNotification({
            open: true,
            message: t("userManagement.notifications.groupDisabled", {
              name: getGroupFullDisplayName(selectedGroup),
            }),
            severity: "success",
          });
    ...
    try {
      await confirm({
        ...
        onConfirm: async () => {
          ...
          setNotification({
            open: true,
            message: t("userManagement.notifications.groupEnabled", {
              name: getGroupFullDisplayName(selectedGroup),
            }),
            severity: "success",
          });
    ...
    try {
      await confirm({
        ...
        onConfirm: async () => {
          ...
          setNotification({
            open: true,
            message: t("userManagement.notifications.groupDeleted", {
              name: getGroupFullDisplayName(selectedGroup),
            }),
            severity: "success",
          });
  ...
  const handleConfirmRenameGroup = async () => {
    ...
    try {
      await groupService.updateGroup(renameGroupDialog.selectedItem.id, {
        name: groupNewName.trim(),
      });
      ...
      const message =
        ...
        t("userManagement.notifications.failedToRenameGroup");
      showError(message);
    }
  };
  ...
  const handleConfirmBulkGroupChange = async () => {
    ...
    try {
      ...
      showSuccess(
        t("userManagement.notifications.bulkGroupChanged", { count: selectedUsers.length })
      );
      clearSelection();
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await confirm({
        ...
        description: t("userManagement.confirmations.bulkDeleteDescription", {
    ...
    try {
      ...
      if (editDialog.selectedItem) {
        ...
        if (!formData.password || formData.password !== formData.password_confirmation) {
          setFormErrors({
            password_confirmation: t("userManagement.notifications.passwordMismatch"),
          });
          return;
        }
  ...
  return (
    <div className="space-y-6 overflow-visible">
          ...
          {selectedGroup && isExpertOrManager(user) && (
            <div className="flex items-center gap-2">
              {selectedGroup.name?.toLowerCase() !== "default" && (
                <>
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent>
                      {selectedGroupDisabled
                        ? t("userManagement.groups.enableGroup")
                        : t("userManagement.groups.disableGroup")}
                    </TooltipContent>
                  </Tooltip>
                </>
              )}
              <Tooltip>
                ...
                <TooltipContent>
                  {selectedGroup.name?.toLowerCase() === "default"
                    ? t("userManagement.groups.defaultCannotDelete")
                    : t("userManagement.groups.deleteGroup")}
                </TooltipContent>
              </Tooltip>
            </div>
          )}
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>
                        {selectedUsers.length > 0
                          ? t("userManagement.actions.deselectAll")
                      ...
                      >
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="text-[10px] text-muted-foreground flex items-center gap-1.5">
                                ...
                                <Tooltip>
                                  ...
                                  <TooltipContent side="top">
                                    {copiedEmail === tableUser.email
                                      ? t("userManagement.actions.copied")
                                      : t("userManagement.actions.copyEmail")}
                                  </TooltipContent>
                                </Tooltip>
                              </div>
                            </div>
                        ...
                        <td className="px-3 py-2">
                          <StatusBadge
                            ...
                            text={
                              tableUser.email_verified
                                ? t("userManagement.emailStatus.verified")
                                : t("userManagement.emailStatus.unverified")
                            }
                          ...
                          <ModelActionGroup
                            actions={[
                              ...
                              ...(isExpertOrManager(user)
                                ...
                                : []),
                              // API tokens green key = active
                              ...(isExpertOrManager(user)
                                ? [
                                    {
                                      key: "api-tokens",
                                      icon: KeyRound,
                                      tooltip: t("apiTokens.manage", "Manage API tokens"),
                                      variant: tableUser.has_api_access
                                        ? ("success" as const)
                                        : ("default" as const),
                                      onClick: () => apiTokensDialog.open(tableUser),
                                    },
                                  ]
                                : []),
                              {
                              ...
                              },
                              {
                                ...
                                onClick: () => {
                                  void handleDeleteUser(tableUser);
                                },
                              },
                            ]}
                            layout="horizontal"
      ...
      <UniversalForm
        ...
        title={
          editDialog.selectedItem
            ...
            : t("userManagement.dialog.addTitle")
        }
        ...
        submitText={
          editDialog.selectedItem
            ? t("userManagement.dialog.updateUser")
            : t("userManagement.dialog.createUser")
        }
        loading={formLoading}
        errors={formErrors}
        maxWidth="lg"
      />
      <ApiTokensDialog
        user={apiTokensDialog.selectedItem}
        isOpen={apiTokensDialog.isOpen}
        onClose={apiTokensDialog.close}
      />
      {isExpertOrManager(user) && (
        <GroupManagement
      ...
      <UniversalForm
        ...
        values={{
          user: changeGroupDialog.selectedItem?.name || "",
      ...
      <UniversalForm
        ...
        values={{
          users: t("userManagement.actions.selected", { count: selectedUsers.length }),
          group_id: bulkSelectedGroupId || "",
        }}
        onChange={(key, value) => {
          if (key === "group_id") {
...
async function fetchUsersWithPagination(params: {
  ...
  try {
    ...
    const response = await axios.get(
      `/users?page=${pageParam}&per_page=${perPageParam}${searchParam}`
    );
...
async function handleGroupChangeConfirmation(params: {
  ...
  try {
    ...
    setUsers((prevUsers) =>
      prevUsers.map((u) => (u.id === movedUserId ? { ...u, group_id: selectedGroupId } : u))
    );

    showSuccess(
      "User moved to new group and logged out. They must re-login to access their new workspace."
    );
    changeGroupDialog.close();
    setSelectedGroupId(null);

    // Refresh in background without blocking UI
    Promise.all([fetchUsers(), fetchGroups()]).catch((err) => {
