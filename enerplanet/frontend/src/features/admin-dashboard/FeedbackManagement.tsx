...
export const FeedbackManagement: React.FC<FeedbackManagementProps> = ({ onFeedbackAction }) => {
  ...
  const rawFeedbacks = feedbackData?.data || [];
  const feedbacks = searchQuery
    ? rawFeedbacks.filter((f) => f.subject.toLowerCase().includes(searchQuery.toLowerCase()))
    : rawFeedbacks;
  const allFeedbacks = useAllFeedback().data || [];
  ...
  const getStatusCounts = () => {
    const pending = allFeedbacks.filter((f: FeedbackItem) => f.status === "pending").length;
    const inProgress = allFeedbacks.filter((f: FeedbackItem) => f.status === "in_progress").length;
    const resolved = allFeedbacks.filter((f: FeedbackItem) => f.status === "resolved").length;
  ...
  ): { items: Array<{ path: string; mime_type: string; size: number }>; fromJson: boolean } => {
    if (feedback.images) {
      try {
          ...
          <div className="flex flex-wrap gap-3">
            <div className="relative">
              <input
                type="text"
                placeholder={
                  t("feedbackManagement.filters.searchPlaceholder") || "Search by subject..."
                }
                value={searchQuery}
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {(feedback.images || feedback.image_path) &&
                              (() => {
                                ...
                                if (feedback.images) {
                                  try {
                                    count = JSON.parse(feedback.images).length;
                                  } catch {
                                    count = 1;
                                  }
                    ...
                    <div className="p-5 space-y-5">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium border ${getStatusColor(selectedFeedback.status)}`}
                          >
                            {getStatusIcon(selectedFeedback.status)}
                            {getStatusLabel(selectedFeedback.status)}
                          </span>
                          <span
                            className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium border ${getCategoryColor(selectedFeedback.category)}`}
                          >
                            {getCategoryIcon(selectedFeedback.category)}
                            {getCategoryLabel(selectedFeedback.category)}
                          </span>
                          <span
                            className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium ${getPriorityColor(selectedFeedback.priority)}`}
                          >
                      ...
                      <div>
                        ...
                        <div
                          className="text-sm p-4 bg-muted/20 rounded-lg border border-border prose prose-sm dark:prose-invert max-w-none
                                  ...
                                  >
                                    {imageLoadFailed[idx] ? (
                                      ...
                                      <img
                                        src={
                                          fromJson
                                            ? getImageUrl(selectedFeedback.id, idx)
                                            : getImageUrl(selectedFeedback.id)
                                        }
                                        ...
                                        onError={() =>
                                          setImageLoadFailed((prev) => ({ ...prev, [idx]: true }))
                                        }
                                      />
                                    )}
                        ...
                        <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                          ...
                          {selectedFeedback.responded_at && (
                            <p className="text-xs text-blue-600 dark:text-blue-400 mt-2">
                              {t("feedbackManagement.viewDialog.respondedOn")}{" "}
                              {formatDateTime24h(selectedFeedback.responded_at)}
                              {selectedFeedback.responded_by_user?.name &&
                                ` • ${selectedFeedback.responded_by_user.name}`}
                            </p>
                          )}
                        </div>
                  ...
                  return (
                    <>
                      <img
                        src={
                          fromJson
                            ? getImageUrl(selectedFeedback.id, lightboxImageIndex)
                            : getImageUrl(selectedFeedback.id)
                        }
                      ...
                      {images[lightboxImageIndex] && (
                        <p className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 text-sm text-white/70">
                          ...
                          {images[lightboxImageIndex].mime_type &&
                            ` • ${images[lightboxImageIndex].mime_type.split("/")[1]?.toUpperCase()}`}
                        </p>
                      )}
                    </>
                  );
