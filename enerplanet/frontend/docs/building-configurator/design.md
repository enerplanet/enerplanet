...
`src/features/configurator/` as a stage-based per-building editor. Background
and the decision to rebuild rather than port the prototype are in the brain
vault: `decisions/2026-08-31-enerplanet-native-building-configurator.md`. The
data it consumes is defined by
`decisions/2026-08-31-configurator-c2t-enrich-endpoint.md`.
...
- Address search is token-limited. The limit was hit during a workshop in Koeln
  ...
  story, recorded in the vault's `wp3-user-stories-mapping.md`, not in
  `user-stories.md` here since it is not a Building Configurator story). Any
  address search in the configurator must be bounded.
...
Full 24-story set with acceptance criteria: `user-stories.md`. Full 38-story
mapping including stories outside the configurator's scope:
`thesis/building-configurator/wp3-user-stories-mapping.md` in the brain vault.
Priority 0 and 1 stories that must be in the first draft:
