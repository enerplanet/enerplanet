...
Everything covers Loenen, Netherlands, the default smoke site. The
City2TABULA and PyLovo fixtures cover the same ground: the box 6.0162 52.0988
to 6.0384 52.1130, which is the extent of the four low-voltage grids PyLovo
ships, so every building with a grid behind it also has 3D data.

PyLovo stores its geometry in EPSG:3035 and City2TABULA in the country-specific
EPSG:28992. Neither is reprojected at load time, because the join between them
...
The City2TABULA restore reads its target from the dependency's own
`environment/docker.env`, so it follows that file rather than duplicating it.
Override any of these to restore elsewhere:

| Variable | Default |
|---|---|
| `C2T_DB_NAME` | `DB_NAME` from `docker.env`, with `_nl` appended |
| `C2T_DB_HOST` | `DB_HOST`, with `host.docker.internal` resolved to `localhost` |
| `C2T_DB_PORT` / `C2T_DB_USER` / `C2T_DB_PASSWORD` | the matching `DB_*` value |

The pylovo restore reads the same way from that checkout's `.env.docker`,
falling back to `.env.example`, with `PYLOVO_DB_NAME`, `PYLOVO_DB_HOST`,
`PYLOVO_DB_PORT`, `PYLOVO_DB_USER` and `PYLOVO_DB_PASSWORD` as the overrides.
